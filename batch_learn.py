...
def comp_loss(b, s, log_price, batch):
...
def grad_b_s(b, s, log_price, batch):
...
def comp_b_s(nn, p, log_price, batch):
    b = np.zeros((DATA_LENGTH, 1), dtype=float)
    s = np.zeros((DATA_LENGTH, 1), dtype=float)
...
def scaled_grad(grad, scales):
    res = []
    for i in range(len(grad)):
        res.append(grad[i] * scales[i])
    return res
...
def gradient_descent(nn, p, log_price, steps, learning_rate):
    ...
    for step in range(steps):
        batch = np.random.randint(DATA_FROM + MA_DAYS, DATA_TO - 1 - INF_CANDLES, size=(BATCH_SIZE, ), dtype=int)
        tic = time.clock()
        b, s = comp_b_s(nn, p, log_price, batch)
        toc = time.clock()
        print('b s compute took ', toc - tic)
        print('step', step)
        tic = time.clock()
        loss = comp_loss(b, s, log_price, batch)
        toc = time.clock()
        print('compute loss took ', toc - tic)
        print('loss', loss)
        tic = time.clock()
        db, ds = grad_b_s(b, s, log_price, batch)
        toc = time.clock()
        ...
        weighted_grad = div(acc_theta_grad, BATCH_SIZE)
        weight_update = mul(weighted_grad, LEARNING_RATE)
        #vanishing gradients
        weight_update_vanishing_grads = scaled_grad(weight_update, [81, 27, 9, 3, 1])
        nn.theta = sum(nn.theta, weight_update_vanishing_grads)
        nn.save('net_batch_vg')
